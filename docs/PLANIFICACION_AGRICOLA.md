...
Cada lote puede tener archivos geograficos asociados en formato `KML` o `KMZ`. Estos archivos sirven para recorridas operativas, visualizacion en mapa y georreferenciacion mobile. La metadata del archivo se guarda en `LoteArchivoGeografico`; el binario se almacena en Supabase Storage y, al registrar el archivo, el backend intenta procesarlo para guardar `geometriaGeoJson` y `superficieCalculadaHa`. Un lote puede tener varios archivos, pero solo uno deberia marcarse como principal para la vista de mapa. Si el archivo no tiene geometria valida, queda auditado con estado `rechazado` y el motivo en observaciones.
...
- las hectareas planificadas de una linea no pueden ser negativas ni superar la superficie total del lote. La UI debe alertar si el usuario excede ese maximo y bloquear el guardado hasta corregirlo; el backend debe rechazar cualquier payload invalido;

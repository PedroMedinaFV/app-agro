...
export function mapearAgriculturaCultivo(cultivo: ErpPadronCultivo, empresaErpId = 'empresa:mock'): ErpCultivo {
  return {
    ...
    actividadErpId: cultivo.idActividad ? `${empresaErpId}:actividad:${cultivo.idActividad}` : undefined,
    idEspecie: cultivo.idEspecie ?? undefined,
    especieErpId: cultivo.idEspecie ? `${empresaErpId}:especie:${cultivo.idEspecie}` : undefined,
    idCampania: cultivo.idCampania ?? undefined,
    campaniaErpId: cultivo.idCampania ? `${empresaErpId}:campania:${cultivo.idCampania}` : undefined,

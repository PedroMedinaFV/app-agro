...
export type PlanificacionBaseProps = {
  ...
  cerrarPlanificacionActiva: (planificacionId: string) => Promise<void>;
  ...
  aplicarProtocoloALineas: (lineaIds: string[], protocoloId: string) => number;
  aplicarDestinoALineas: (lineaIds: string[], destinoVenta: string) => number;
  aplicarRindeALineas: (lineaIds: string[], rindeEstimado: number) => number;
  eliminarLineaPlanificacion: (lineaId: string) => void;
  eliminarLineasPlanificacion: (lineaIds: string[]) => number;
